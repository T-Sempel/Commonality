...
export default function CommonalityApp() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (s) {
        const user = await safeGet(`users:${s.userId}`);
        if (user) {
          setSession({ ...s, user });
          setScreen(s.role === "moderator" ? "modDashboard" : "home");
          // Load per-user settings
          const userSettings = await safeGet(`settings:${s.userId}`);
  ...
  const updateSettings = async (patch) => {
    ...
    setSettings(next);
    if (session?.userId) await safeSet(`settings:${session.userId}`, next);
  };
  ...
  const handleLogin = async (userId, role) => {
    ...
    await safeSet("session", s);
    const user = await safeGet(`users:${userId}`);
    setSession({ ...s, user });
    setScreen(role === "moderator" ? "modDashboard" : "home");
    const userSettings = await safeGet(`settings:${userId}`);
    setSettings(userSettings || defaultSettings());
  };
...
function AuthScreen({ onLogin, showToast }) {
  ...
  const [step, setStep] = useState("landing"); // landing | login | signup | tos | otp | mod | tosView
  ...
  const handleSendOtp = async (forMode) => {
    ...
    for (const k of userKeys) {
      const u = await safeGet(k);
      if (u?.email?.toLowerCase() === trimmed) { existing = u; break; }
    }
    if (m === "login" && !existing) {
      showToast("No account found. Try signing up instead.", "error"); return;
    }
    if (m === "signup" && existing) {
    ...
    if (!user) {
      ...
      await safeSet(`users:${user.id}`, user);
    }
  ...
  const handleVerifyOtp = async () => {
    if (otp !== sentOtp) { showToast("Incorrect code", "error"); return; }
    const user = await safeGet(`users:${pendingUserId}`);
    if (user?.banned) { showToast("This account has been suspended.", "error"); return; }
    onLogin(pendingUserId, "user");
  };

  const handleModLogin = async () => {
    if (modCode !== MOD_CODE) { showToast("Invalid moderator code", "error"); return; }
    let modUser = await safeGet("users:moderator");
    if (!modUser) {
      modUser = { id: "moderator", email: "mod@commonality.app", provider: "system", handle: "Moderator", createdAt: now(), role: "moderator" };
      await safeSet("users:moderator", modUser);
    }
    onLogin("moderator", "moderator");
  };

  const allAgreed = tosAgreed && houseRulesAgreed && ageConfirmed;

  // ---------- LANDING (chooses login vs signup) ----------
  if (step === "landing") {
    return (
  ...
  if (step === "login") {
    return (
      <div className="px-6 pt-12 pb-8 min-h-screen flex flex-col">
        ...
        <label className="text-xs uppercase tracking-wider mb-2 font-medium" style={{ color: "var(--text-tertiary)" }}>Email</label>
        <input
          value={email}
          onChange={e => setEmail(e.target.value)}
          onKeyDown={e => e.key === "Enter" && handleSendOtp("login")}
        ...
        <button
          ...
          className="mt-6 w-full py-3.5 rounded-xl font-medium text-white transition hover:opacity-90"
          style={{ background: "var(--sage)" }}
        >
          Send sign-in code
        </button>
  ...
  // ---------- SIGNUP (warmer, more inviting, sets expectations) ----------
  if (step === "signup") {
    return (
      <div className="px-6 pt-12 pb-8 min-h-screen flex flex-col" style={{
        background: "linear-gradient(180deg, var(--sage-bg) 0%, var(--bg-app) 40%)",
      }}>
        <button onClick={() => setStep("landing")} className="text-sm flex items-center gap-1 self-start mb-8 hover:opacity-70" style={{ color: "var(--text-tertiary)" }}>
          <ChevronRight size={14} className="rotate-180" /> Back
        </button>
        ...
        <div className="mb-8">
          <div className="text-xs uppercase tracking-[0.15em] mb-2 font-medium" style={{ color: "var(--sage-mid)" }}>
            Step 1 of 3
          </div>
          <div className="font-display text-3xl leading-tight" style={{ color: "var(--sage)" }}>
            Let's start with your email
          </div>
          <div className="text-sm mt-3 leading-relaxed" style={{ color: "var(--text-secondary)" }}>
        ...
        <div className="rounded-2xl p-4 mb-6" style={{ background: "var(--bg-card)", border: "0.5px solid var(--border-mid)" }}>
          <label className="text-xs uppercase tracking-wider mb-2 font-medium block" style={{ color: "var(--text-tertiary)" }}>Email address</label>
          <input
            value={email}
            onChange={e => setEmail(e.target.value)}
          ...
          <div className="mt-4 pt-4 space-y-2 text-xs" style={{ borderTop: "0.5px solid var(--border-soft)", color: "var(--text-secondary)" }}>
            <div className="flex items-start gap-2">
              ...
              <span>Your email stays on Anthropic's servers. Other users see only your anonymous handle.</span>
            </div>
            <div className="flex items-start gap-2">
  ...
  if (step === "tos") {
    return (
      <div className="px-6 pt-12 pb-8 min-h-screen flex flex-col">
        <button onClick={() => setStep("signup")} className="text-sm flex items-center gap-1 self-start mb-6 hover:opacity-70" style={{ color: "var(--text-tertiary)" }}>
          <ChevronRight size={14} className="rotate-180" /> Back
        </button>
        ...
        <div className="space-y-3 mb-6">
          ...
          />
          <AgreeRow
            checked={houseRulesAgreed}
            onChange={setHouseRulesAgreed}
          ...
          <AgreeRow
            ...
            body={
              <span>
                I understand that my email is stored privately, my conversations are subject to moderation review when reported, and I can delete my account at any time.{" "}
                <button onClick={() => setStep("tosView")} className="underline" style={{ color: "var(--sage-mid)" }}>
                  Read the full terms
                </button>
              </span>
            }
        ...
        <button
          ...
          style={{ background: "var(--sage)" }}
        >
          {allAgreed ? "Agree & continue" : "Check all boxes to continue"}
          {allAgreed && <ChevronRight size={15} />}
        </button>

        <div className="text-[11px] mt-3 text-center leading-relaxed" style={{ color: "var(--text-tertiary)" }}>
          By continuing you confirm the items above are true and accept our terms.
        </div>
      </div>
    );
  }
  ...
  if (step === "tosView") {
    return (
      <div className="px-6 pt-8 pb-8 min-h-screen flex flex-col">
        <button onClick={() => setStep("tos")} className="text-sm flex items-center gap-1 self-start mb-6 hover:opacity-70" style={{ color: "var(--text-tertiary)" }}>
          <ChevronRight size={14} className="rotate-180" /> Back to agreements
        </button>

        <div className="font-display text-2xl mb-1" style={{ color: "var(--sage)" }}>Terms & Privacy</div>
        <div className="text-xs mb-6" style={{ color: "var(--text-tertiary)" }}>Version 1.0 · Last updated {new Date().toLocaleDateString()}</div>

        <div className="space-y-5 text-sm leading-relaxed pb-4" style={{ color: "var(--text-secondary)" }}>
          <TosBlock title="What this app is">
            Commonality is a conversation app that pairs you anonymously with other users based on shared traits, then surfaces one difference once a conversation has built some trust. It is not a dating app, debate platform, or social network.
          </TosBlock>
          <TosBlock title="Anonymity">
            Other users see only your randomly generated handle and the profile traits you opt in to share. They never see your email, login provider, real name, or any other identifying information. The platform retains your email for sign-in and abuse prevention.
          </TosBlock>
          <TosBlock title="What you agree not to do">
            Don't post hate speech, slurs, threats, harassment, or content sexualizing minors. Don't share other users' personal information. Don't use the app to recruit, market, scam, or solicit. Don't try to identify other users outside the app. Don't share your account with others.
          </TosBlock>
          <TosBlock title="Moderation">
            Reported messages are reviewed by human moderators. Moderators see only the content needed for enforcement (the reported user's recent messages, conversation context, severity, and reason) — not your email, login provider, or full profile. Severe violations may result in account suspension or permanent block. You can appeal a suspension; see the in-app help.
          </TosBlock>
          <TosBlock title="Data we keep">
            Your email, anonymous handle, account creation date, profile fields you fill in, conversations you participate in, blocks you create, and reports filed by or against you. We keep this until you delete your account, at which point all of the above is permanently removed.
          </TosBlock>
          <TosBlock title="Data we don't keep">
            We don't track your location beyond what you optionally enter as a region type. We don't sell your data. We don't show third-party ads.
          </TosBlock>
          <TosBlock title="Your rights">
            Delete your account at any time from Settings. Export your data on request. Withdraw consent for sensitive profile fields field-by-field. Contact us about your data at privacy@commonality.app.
          </TosBlock>
          <TosBlock title="Changes">
            If we materially change these terms, you'll be asked to re-agree on next sign-in. Minor wording changes will be noted with a new version number above.
          </TosBlock>
        </div>

        <button onClick={() => setStep("tos")} className="mt-6 w-full py-3 rounded-xl font-medium" style={{ background: "var(--bg-soft)", color: "var(--text-primary)" }}>
          Back to agreements
        </button>
      </div>
    );
  ...
  if (step === "otp") {
    ...
    return (
      <div className="px-6 pt-12 pb-8 min-h-screen flex flex-col">
        <button onClick={() => { setStep(isSignup ? "tos" : "login"); setOtp(""); setSentOtp(null); }} className="text-sm flex items-center gap-1 self-start mb-6 hover:opacity-70" style={{ color: "var(--text-tertiary)" }}>
          <ChevronRight size={14} className="rotate-180" /> Back
        </button>
        ...
        <div className="p-4 rounded-xl mb-5" style={{ background: "var(--amber-bg)", border: "0.5px solid var(--amber-border)" }}>
          <div className="text-[10px] uppercase tracking-wider mb-1 font-medium" style={{ color: "var(--amber)" }}>Demo prototype · your code</div>
          <div className="flex items-center justify-between gap-3">
            <div className="font-mono text-2xl tracking-[0.3em]" style={{ color: "var(--amber)" }}>{sentOtp}</div>
            <button
          ...
          <div className="text-[11px] mt-2" style={{ color: "var(--amber)", opacity: 0.85 }}>In production this would be sent to your inbox.</div>
        </div>
        ...
        <button
          ...
          className="mt-6 w-full py-3.5 rounded-xl font-medium text-white transition disabled:opacity-40"
          style={{ background: "var(--sage)" }}
        >{isSignup ? "Verify & finish setup" : "Verify"}</button>
  ...
  if (step === "mod") {
    return (
      <div className="px-6 pt-12 pb-8 min-h-screen flex flex-col">
        ...
        <button onClick={handleModLogin} className="mt-6 w-full py-3.5 rounded-xl font-medium text-white" style={{ background: "var(--amber)" }}>
          Enter dashboard
        </button>
      </div>
    );
...
function AgreeRow({ checked, onChange, title, body }) {
  return (
    <button
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="text-sm font-medium" style={{ color: checked ? "var(--sage)" : "var(--text-primary)" }}>{title}</div>
          <div className="text-xs mt-1 leading-relaxed" style={{ color: "var(--text-secondary)" }}>{body}</div>
        </div>
      </div>
    </button>
  );
...
function TosBlock({ title, children }) {
  return (
    <div>
...
function HomeScreen({ session, setScreen, onLogout, showToast, settings }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setProfile(p);
      const chatKeys = await safeList("chats:");
      const mine = [];
      for (const k of chatKeys) {
        const c = await safeGet(k);
        if (c?.participants?.includes(session.userId) && !c.endedBy) mine.push(c);
      }
      setActiveChats(mine);
    })();
  }, [session.userId]);
...
function ProfileScreen({ session, setScreen, showToast }) {
  ...
  const save = async () => {
    setSaving(true);
    await safeSet(`profiles:${session.userId}`, profile);
    setSaving(false);
...
function MatchesScreen({ session, setScreen, setActiveChatId, showToast, settings }) {
  ...
  // Seed demo users so the prototype is interactive even on a fresh install
  const seedDemoUsers = async () => {
    const seeds = [
      { handle: "Curious River", profile: { ageRange: { value: "25-34", optInMatch: true }, tvLike: { value: "Comedy/sitcoms", optInMatch: true, optInReveal: true }, hobby: { value: "Reading", optInMatch: true, optInReveal: true }, politics: { value: "Liberal", optInMatch: false, optInReveal: true }, food: { value: "Italian", optInMatch: true } } },
      { handle: "Steady Compass", profile: { ageRange: { value: "35-44", optInMatch: true }, tvLike: { value: "Comedy/sitcoms", optInMatch: true }, hobby: { value: "Cooking", optInMatch: true, optInReveal: true }, politics: { value: "Conservative", optInMatch: false, optInReveal: true }, region: { value: "Rural", optInMatch: true, optInReveal: true } } },
      { handle: "Quiet Lantern", profile: { hobby: { value: "Reading", optInMatch: true, optInReveal: true }, food: { value: "Asian", optInMatch: true }, religion: { value: "Buddhist", optInMatch: false, optInReveal: true }, pets: { value: "Cat person", optInMatch: true, optInReveal: true } } },
      { handle: "Bright Field", profile: { tvLike: { value: "Sci-fi/fantasy", optInMatch: true }, hobby: { value: "Gaming", optInMatch: true, optInReveal: true }, education: { value: "Master's", optInMatch: true, optInReveal: true }, politics: { value: "Libertarian", optInMatch: false, optInReveal: true } } },
    ];
    for (const seed of seeds) {
      const id = `demo_${seed.handle.replace(/\s/g,"_").toLowerCase()}`;
      const existing = await safeGet(`users:${id}`);
      if (!existing) {
        await safeSet(`users:${id}`, { id, email: `${id}@demo.local`, provider: "system", handle: seed.handle, createdAt: now() - 86400000, banned: false });
        await safeSet(`profiles:${id}`, seed.profile);
      }
    }
  };
  ...
  useEffect(() => {
    (async () => {
      if (!seeded) { await seedDemoUsers(); setSeeded(true); }
      const myProfile = await safeGet(`profiles:${session.userId}`) || {};
      setProfile(myProfile);
      const blocked = (await safeGet(`blocks:${session.userId}`)) || [];
      const userKeys = await safeList("users:");
      const cands = [];
      for (const k of userKeys) {
        const u = await safeGet(k);
        ...
        const theirProfile = await safeGet(`profiles:${u.id}`) || {};
        ...
        if (shared.length >= 1 && theirRevealable.length >= 1) {
          // Pick the future-difference category
  ...
  const proposeMatch = async (cand) => {
    ...
    const match = {
      ...
      proposedAt: now(),
      aAgreed: true,
      bAgreed: true, // simulated mutual agreement for demo
      bothAgreed: true,
    };
    await safeSet(`matches:${matchId}`, match);
  ...
  const openChat = async (matchId, cand) => {
    ...
    let chat = await safeGet(`chats:${chatId}`);
    if (!chat) {
      // Determine reveal key — must be opt-in-reveal on BOTH sides if exists
      const myProfile = await safeGet(`profiles:${session.userId}`) || {};
      const theirProfile = await safeGet(`profiles:${cand.user.id}`) || {};
      let revealKey = cand.futureCat.key;
      // verify both opted in for reveal on this key
...
function ChatScreen({ session, setScreen, activeChatId, showToast, settings }) {
  ...
  useEffect(() => {
    ...
    // If user requires both-ready confirmation, only that path unlocks
  ...
  const doReveal = async () => {
    if (!chat || chat.unlocked) return;
    const updated = { ...chat, unlocked: true, unlockedAt: now() };
  ...
  const send = async () => {
    ...
    if (harassment) {
      ...
      await safeSet(`reports:${reportId}`, {
      ...
      const updatedUser = { ...session.user, warnings: (session.user.warnings || 0) + 1 };
      await safeSet(`users:${session.userId}`, updatedUser);
    ...
    if (piiViolation) {
      if (piiSensitivity === "strict") {
        // Strict mode: block the send entirely and warn
        showToast("Message blocked: contains personal info. Edit and try again.", "error");
        return;
      } else if (piiSensitivity === "lenient") {
        // Lenient mode: send as-is but flag a warning to the sender only
    ...
    const msg = { id: genId(), from: session.userId, text: displayText, at: now(), warning };
    ...
    // Simulate the other person replying for demo realism
    setTimeout(() => simulateReply(updated), 1200 + Math.random() * 1500);
  };

  const simulateReply = async (currentChat) => {
    if (!currentChat) return;
    const otherId = currentChat.participants.find(p => p !== session.userId);
    const replies = currentChat.unlocked ? [
    ...
    const fresh = await safeGet(`chats:${chatId}`);
    if (!fresh || fresh.endedBy) return;
    const msg = { id: genId(), from: otherId, text: reply, at: now() };
    const updated = { ...fresh, messages: [...fresh.messages, msg] };
    await safeSet(`chats:${chatId}`, updated);
    setChat(updated);
  };
  ...
  const confirmReady = async () => {
    if (!chat) return;
    const updated = { ...chat, readyConfirmed: { ...chat.readyConfirmed, [session.userId]: true, [chat.participants.find(p=>p!==session.userId)]: true } };
    await safeSet(`chats:${chatId}`, updated);
    setChat(updated);
  };
  ...
  const leave = async (skipConfirm) => {
    ...
    const savedBy = autoSave && !chat.savedBy.includes(session.userId)
      ...
      : chat.savedBy;
    const updated = { ...chat, endedBy: session.userId, endedAt: now(), savedBy };
    await safeSet(`chats:${chatId}`, updated);
  ...
  const save = async () => {
    ...
    await safeSet(`chats:${chatId}`, { ...chat, savedBy });
  ...
  const block = async () => {
    ...
    const blocks = await safeGet(`blocks:${session.userId}`) || [];
    if (!blocks.includes(otherId)) blocks.push(otherId);
    await safeSet(`blocks:${session.userId}`, blocks);
    await safeSet(`chats:${chatId}`, { ...chat, endedBy: session.userId, endedAt: now() });
...
function SavedChatsScreen({ session, setScreen, setActiveChatId }) {
  ...
  useEffect(() => {
    (async () => {
      const keys = await safeList("chats:");
      const mine = [];
      for (const k of keys) {
        const c = await safeGet(k);
        if (c?.participants?.includes(session.userId)) mine.push(c);
      }
...
function ModeratorDashboard({ session, onLogout, showToast }) {
  ...
  const refresh = async () => {
    setLoading(true);
    const keys = await safeList("reports:");
    const all = [];
    for (const k of keys) {
  ...
  const takeAction = async (action, note) => {
    ...
    await safeSet(`reports:${selected.id}`, updated);

    if (action === "warn") {
      const u = await safeGet(`users:${selected.reportedUser}`);
      if (u) await safeSet(`users:${selected.reportedUser}`, { ...u, warnings: (u.warnings||0)+1 });
    }
    if (action === "suspended" || action === "blocked") {
      const u = await safeGet(`users:${selected.reportedUser}`);
      if (u) await safeSet(`users:${selected.reportedUser}`, { ...u, banned: true, bannedAt: now(), bannedReason: selected.reason });
      // also end any active chats
      const chat = await safeGet(`chats:${selected.chatId}`);
      if (chat && !chat.endedBy) {
        await safeSet(`chats:${selected.chatId}`, { ...chat, endedBy: "moderator", endedAt: now() });
      }
    }
  ...
  const reinstateUser = async (userId, sourceReport, note) => {
    const u = await safeGet(`users:${userId}`);
    if (!u) { showToast("User not found", "error"); return; }
    await safeSet(`users:${userId}`, { ...u, banned: false, reinstatedAt: now(), bannedReason: null });
    ...
    for (const k of keys) {
      const r = await safeGet(k);
      if (!r) continue;
      if (r.reportedUser === userId && (r.status === "suspended" || r.status === "blocked")) {
        const auditEntry = {
          action: "reinstated",
          by: session.userId,
          byHandle: session.user.handle,
    ...
    if (sourceReport) {
      const fresh = await safeGet(`reports:${sourceReport.id}`);
      setSelected(fresh);
    }
    refresh();
  };
...
function ReportDetail({ report, onBack, onAction, onReinstate }) {
  ...
  useEffect(() => {
    (async () => {
      setChat(await safeGet(`chats:${report.chatId}`));
      const u = await safeGet(`users:${report.reportedUser}`);
...
function SettingsScreen({ session, setScreen, settings, updateSettings, setSession, showToast, resolvedTheme, onLogout }) {
  ...
  const loadBlocked = async () => {
    const blockedIds = (await safeGet(`blocks:${session.userId}`)) || [];
    const users = [];
    for (const bid of blockedIds) {
      const u = await safeGet(`users:${bid}`);
  ...
  const loadCounts = async () => {
    ...
    for (const k of keys) {
      const c = await safeGet(k);
      if (c?.participants?.includes(session.userId)) {
  ...
  const unblock = async (uid) => {
    const blocks = ((await safeGet(`blocks:${session.userId}`)) || []).filter(b => b !== uid);
    await safeSet(`blocks:${session.userId}`, blocks);
  ...
  const regenerateHandle = async () => {
    const updated = { ...session.user, handle: genHandle() };
    await safeSet(`users:${session.userId}`, updated);
    setSession(prev => ({ ...prev, user: updated }));
    setNewHandle(updated.handle);
    showToast(`New handle: ${updated.handle}`, "success");
  };
  ...
  const deleteAccount = async () => {
    ...
    await window.storage.delete(`users:${session.userId}`).catch(() => {});
    await window.storage.delete(`profiles:${session.userId}`).catch(() => {});
    await window.storage.delete(`settings:${session.userId}`).catch(() => {});
    await window.storage.delete(`blocks:${session.userId}`).catch(() => {});
    const chatKeys = await safeList("chats:");
    for (const k of chatKeys) {
      const c = await safeGet(k);
      if (c?.participants?.includes(session.userId)) {
    ...
    <div className="pb-12">
      ...
      <div className="px-6 space-y-5">
        ...
        <Section icon={<UserCircle size={14} />} title="Account">
          ...
          <InfoRow label="Email" value={session.user.email} hint="Only Anthropic-side; never shared with other users." />
